...
class SimpleS3Client extends S3Client
{
    ...
    public function upload(string $bucket, string $key, mixed $object, array $options = []): PutObjectOutput|CompleteMultipartUploadOutput
    {
        ...
        if (null !== $contentLength && $contentLength < 64 * $megabyte) {
            return $this->doSmallFileUpload($options, $bucket, $key, $object);
        }
        ...
        if ($chunkIndex > 0) {
            if (empty($uploadId)) {
                 ...
                 */
                return $this->doSmallFileUpload($options, $bucket, $key, $buffer);
            }
        ...
        if (empty($parts)) {
            ...
            return $this->doSmallFileUpload($options, $bucket, $key, '');
        }
    ...
    private function getStream(mixed $object, int $chunkSize): FixedSizeStream
    {
        return FixedSizeStream::create(StreamFactory::create($object, $chunkSize), $chunkSize);
    }

    private function doMultipartUpload(string $bucket, string $key, string $uploadId, int $partNumber, mixed $buffer): CompletedPart
    {
        try {
    ...
    private function doSmallFileUpload(array $options, string $bucket, string $key, mixed $object): PutObjectOutput
    {
        return $this->putObject(array_merge($options, [
            'Bucket' => $bucket,
            'Key' => $key,
            'Body' => $object,
        ]));
