    ...
    <div class="row">
        ...
        <div class="col-3">
            <div id="lists" class="list-group sticky-top bg-body rounded" style="top: 1rem">
                <?php foreach ($data['_ordered'] as $schema => $routeGroup) { if (str_starts_with($schema, '_')) continue; ?>
                    <a class="list-group-item disabled"><i class="fa-solid fa-layer-group"></i><span><?php echo $schema; ?></span></a>
                    <?php foreach ($routeGroup as $groupName => $routes) { ?>
                      <a class="list-group-item list-group-item-action" href="#<?php echo str_replace(' ', '', $groupName) ?? '/'; ?>"><i class="fa-solid fa-arrow-right fa-sm" style="color: #c1c9d7;"></i><span><?php echo $groupName ?? '/'; ?></span></a>
                  <?php } ?>
                <?php } ?>
            </div>
        </div>
        ...
        <div class="col-9">
            <?php foreach ($data['_ordered'] as $schema => $routeGroup) { if (str_starts_with($schema, '_')) continue; ?>
              <div class="card mb-3 bg-transparent">
                <div class="card-header" style='background: #dee2e6'><i class="fa-solid fa-layer-group"></i> <span class='ms-2'><?php echo ucfirst($schema) ?></span></div>
                <div class="card-body pb-0">
                    <?php foreach ($routeGroup as $groupName => $routes) { if (str_starts_with($groupName, '_')) continue; ?>
                      <div class="group-item">
                          <?php if (!empty($groupName)) { ?><h5 class="title" id="<?php echo str_replace(' ', '', $groupName); ?>"><?php echo $groupName; ?></h5><?php } ?>
