        ...
        <q-list v-ripple="false">
          ...
          <q-item class="panel-head q-mb-sm bg-primary shadow-bottom">
            ...
            <q-item-section side top>
              <div>
                <q-btn flat round color="white" size="12px" :icon="mdiBellPlus"
                       @click="onClickInstall"
                       v-if="access.permission !== true && this.support"
                ></q-btn>
                <q-btn color="white" size="12px" flat round :icon="mdiRefresh" @click="loadData">
                  <q-tooltip>{{ $t('Refresh') }}</q-tooltip>
                </q-btn>
                <q-btn color="white" size="12px" flat round :icon="mdiCheckAll" @click="onReadAllItem">
          ...
          <!--Items-->
          <q-item :key="item.id" clickable class="cursor-pointer item" active-class="text-blue"
                  v-for="item in getData"
                  @click="onReadItem(item); onOpenItem(item);"
                  :active="!item.readed"
          >
            <q-item-section>
            ...
            <q-item-section side class="q-pl-none">
              <q-btn @click.stop="onRemoveItem(item)" size="sm" flat round color="red" :icon="mdiDeleteOutline">
          ...
          <div class="full-width flex justify-center q-my-xs">
            <q-btn v-if="resp.pager?.next" @click="onNextPage()" :label="$t('Load More')" size="11px" :icon="mdiRefresh" flat></q-btn>
          </div>
        </q-list>
      </q-scroll-area>
    </q-drawer>
...
export default defineComponent({
  ...
  methods: {
    ...
    onShowPanel() {
      if (!this.resp.pager) this.loadData();
    },
    onNextPage() {
    ...
    onOpenItem(item: NotificationResource) {
    ...
    initNotification() {
      if (!this.support || !process.env.FIREBASE_APIKEY) return;
    ...
    accessNotification(status) {
      if (!status) return (this.access.permission = false);
    ...
    async installFirebase() {
      ...
      getToken(this.firebase.messaging).then((token) => {
        if (token) this.saveFirebaseToken(token);
      });
    ...
    onFirebaseMessage(payload: any) {
      const notification = JSON.parse(payload.data.item);

      // Append
      if (notification.id !== null) {
        if (!this.resp.data) this.resp.data = [];
        this.resp.data.unshift(notification);
        this.unreadCount++;
      }

      // Open
      this.onOpenItem(notification);
    },
    onClickInstall() {
      this.$appStore
        .confirmPromise(
