<template>
  ...
  >
    ...
    <template #top-selection v-if="header && selectedRows.length > 0">
      <q-btn-group v-if="!$q.screen.xs">
        <q-btn color="red" size="12px" v-close-popup v-if='deleteProp' :icon="mdiDeleteOutline" @click="onActionRemoveAll(this.selectedRows)"><q-tooltip>Tümünü Sil</q-tooltip></q-btn>
    ...
    <template #top-right v-if="header">
      <div class="row q-gutter-sm">
        <q-btn-group v-if="!$q.screen.xs">
          <q-btn v-if="refreshButton" color="primary" v-close-popup :icon="mdiRefresh" size="12px" @click="refresh"><q-tooltip>{{ $t('Refresh') }}</q-tooltip></q-btn>
          <q-btn v-if="exportButton && getExportedColumns.length > 0" color="primary" v-close-popup :icon="mdiFileExportOutline" size="12px" @click="$refs.exporter.toggle()"><q-tooltip>{{ $t('Export') }}</q-tooltip></q-btn>
        ...
        >
          <div class="column q-gutter-sm">
            <q-btn v-if="refreshButton" v-close-popup color="primary" :icon="mdiRefresh" size="12px" @click="refresh"><q-tooltip>{{ $t('Refresh') }}</q-tooltip></q-btn>
            <q-btn v-if="exportButton && getExportedColumns.length > 0" color="primary" v-close-popup :icon="mdiFileExportOutline" size="12px" @click="$refs.exporter.toggle()"><q-tooltip>{{ $t('Export') }}</q-tooltip></q-btn>
            <slot name="tableActions"></slot>
          </div>
        </q-btn-dropdown>
    ...
    <template #header-cell='props'>
      <q-th :props="props">
        ...
        <!--Filters-->
        <q-btn v-if='getColumnFilter.hasOwnProperty(props.col.name) || $slots["filter_" + props.col.name]' size='11px' class='q-ml-xs' flat dense rounded
               :icon='![null, undefined, ""].includes(filterValues[props.col.name]) ? mdiFilter : mdiFilterOutline' @click.stop
        ...
        >
          <q-menu>
            <div class="q-px-md q-py-sm">
              <TableFilter v-if='$slots["filter_" + props.col.name]' :filter='getColumnFilter[props.col.name]' :column='props.col' v-model='filterValues[props.col.name]' @onSearch='refresh'>
                <slot :name='"filter_" + props.col.name' :column='props.col' :values='filterValues'></slot>
              </TableFilter>

              <TableFilter v-else :filter='getColumnFilter[props.col.name]' :column='props.col' v-model='filterValues[props.col.name]' @onSearch='refresh'></TableFilter>
            </div>
          </q-menu>
        </q-btn>
    ...
    <template #pagination="props">
      ...
      <q-btn
        ...
        flat
      />
      <q-btn :disable="props.isFirstPage" :style='[props.isFirstPage ? "opacity: 0.4 !important" : ""]' @click="props.prevPage" :icon="mdiChevronLeft" round dense flat />
      <q-btn :disable="props.isLastPage" :style='[props.isLastPage ? "opacity: 0.4 !important" : ""]' @click="props.nextPage" :icon="mdiChevronRight" round dense flat />
      <q-btn
        v-if="props.pagesNumber > 2 && props.pagination.isTotal"
  ...
  <SimpleDialog ref='exporter' v-if='exportButton'>
    ...
    <template #content>
      <q-option-group ref='exportItem' class='table-exporter' v-model="exportedColumns" :options="getExportedColumns" type="checkbox" inline />
    </template>
    <template #actions>
      <q-btn flat label="Tümünü Seç" color="primary" :icon='mdiCheckAll' @click='selectAllExportedColumns()'/>
...
export default defineComponent({
  ...
  computed: {
    getColumns() {
      let all = this.columns.map((c) => {
        return {
          ...c,
          ...{ field: c.name, label: c.label },
          ...this.columnConfig.hasOwnProperty(c.name) ? this.columnConfig[c.name] : {}
        };
      });
    ...
    getColumnFilter() {
      ...
      return {...filters, ...this.columnFilter}
    },
    getDefaultSortBy() {
      return this.columns
        .find((c) => (c.hasOwnProperty('sortable_default')))?.name || 'id'
    },
    getDefaultSortDescending() {
      return this.columns
        .find((c) => (c.hasOwnProperty('sortable_desc')))?.sortable_desc || false
    }
  },
  mounted() {
  ...
  methods: {
    ...
    onExport(type) {
      this
        .requestProp({...this.getQuery(), ...{ export: type, export_field: this.exportedColumns }}, { responseType: 'blob' })
